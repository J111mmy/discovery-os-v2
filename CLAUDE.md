...
**Handoff law — JIMMY DOES NOT CODE. Give commands, not instructions (set by Jimmy 2026-07-19, after repeated failures):** every action item aimed at Jimmy MUST ship with the exact copy-pasteable command block. NEVER write "merge PR #163", "apply migration 0040", "rebase that branch", or "tell Codex to fix it". Write the literal `gh`/`git`/shell line, including `-R J111mmy/discovery-os-v2`. Numbered steps, in order, each with what he should SEE afterwards so he can tell pass from fail. Anything that needs the Supabase dashboard or a UI gets click-by-click steps, not "apply it". Every handoff to Codex/Design/Fable ships as a quoted block he can copy wholesale. NO `#` comment lines inside command blocks: his interactive zsh executes them and errors. Assuming he will fill in the gaps wastes his time and stalls the work.

**Process & Docs discipline (set by Jimmy 2026-08-01):**
- **Agent lanes are strict.** Codex = backend / SQL / migrations / security gates. **DiscOS-Design** = frontend backlog issues. **Fable-Design** = experimental design (front-of-house / Present mode, PR #136) — **OFF-LIMITS** to every other agent. Do NOT route frontend work to Codex when the Design agents are active; do NOT touch, rebase, preview, or recommend merging Fable-Design's experimental branches. Frontend goes to the Design agents.
- **Worktree isolation.** Every agent works in its OWN git worktree, never the shared checkout. Never `git add -A`; stage explicit paths.
- **`main` is production** (auto-deploys via Vercel). Never push to `main` directly. Merge via PR with green CI (the #150 gate) + review. Branch protection enforces this.
- **Non-prod work uses the staging database (#33), never production** — so experiments and previews cannot touch customer data.
- **Docs are part of the definition of done.** Read `CLAUDE.md`, `docs/architecture/ONTOLOGY.md`, and `docs/architecture/AGENT_STANDARDS.md` at session start. If a change alters documented behaviour, update the docs in the SAME PR. Doc accuracy is checked at review. A "closed" issue is not proof something shipped — verify against `main` (see #59, closed citing an unmerged commit).
...
**→ Full agent standard: `docs/architecture/AGENT_STANDARDS.md` (READ before adding or editing any Inngest LLM agent).** It is the canonical, mechanically-enforced rulebook (R1–R9: chunk sub-60s, cap premium `maxTokens`, timeout < 60s, `retries ≤ 1`, double-spend guard, cost ceiling, golden-eval detection) plus an author checklist and a live compliance matrix. The rules are enforced by the agent-standards CI guard (#113) — a doc nobody enforces is worthless, so every rule maps to a build-breaking check, not a reviewer's memory. When you change an agent, update the compliance matrix in that doc.
