...
- [ ] **SEC-RLS-2 (Low→Med):** ~33 policies still use the inline `SELECT ... FROM org_members` check instead of the helpers (full list captured in assessment). Only the `org_invites` one formed a recursion cycle (→ fixed in 0023); the rest query `org_members` once so they don't recurse today — but each is a latent footgun. **Deliberately NOT bundled into 0024** — it's a larger mechanical change deserving its own migration + read/write verification. Follow-up task.
...
- [ ] `0025_standardize_membership_policies.sql` — SEC-RLS-2 (~33 policies → helpers). **Codex Task D** authors now; APPLY only after Opus reviews before/after dump AND after Jimmy's UI invite test.
