...
- **P3: presentation polish.** Slide-per-section layout option, speaker notes from the artifact's structure trace, a "present from spine" project briefing mode.
...
- **P5: prototypes lane.** Store prototype links/embeds (v0/Figma/Bolt outputs) as a first-class artifact kind, traced to the problems that motivated them (ties to the existing evidence-driven prototype pipeline idea, #80).
- **P6: freshness engine.** "Out of date: N new sources" chips already exist; add one-click regenerate with cost shown before confirm (#74 + #76 envelope).
