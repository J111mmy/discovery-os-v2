    ...
    <main className="min-h-screen px-5 py-8 sm:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mb-8">
          ...
          {people.length === 0 ? (
            <div className="rounded-xl border border-[var(--line)] bg-[var(--surface)] p-8 text-center text-sm text-[var(--ink-2)]">
            ...
            <div className="grid gap-2">
              {people.map((person) => (
                <Link
                  ...
                  className="flex items-center justify-between gap-4 rounded-lg border border-[var(--line)] bg-[var(--surface)] px-4 py-3 transition-colors hover:border-[var(--accent)]"
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-[var(--ink)]">{person.name}</p>
                    {(person.role || person.email) && (
                      <p className="mt-1 truncate text-xs text-[var(--ink-2)]">
        ...
        <section className="mb-8">
          ...
          {projects.length === 0 ? (
            <p className="text-sm text-[var(--ink-2)]">Not linked to any projects yet.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {projects.map((project) => (
                <Link
                  ...
                  className="rounded-full border border-[var(--line)] px-3 py-1 text-sm text-[var(--ink-2)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]"
                >
                  {project.name}
                </Link>
              ))}
